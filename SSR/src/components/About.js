...
class About extends Component {
	...
	isAtMiddleOfElement(element) {
  	return (element.getBoundingClientRect().bottom - (element.clientHeight/2)) <= window.innerHeight;
	}
	...
	async componentDidMount() {
		...
		if(!isBrowser) {
			document.getElementById("mtdSphereVideoContainer").style.backgroundImage = `url(${mtdSphereVideoAnimation_still})`;
		}
    ...
    return (
			<div id="aboutWrap">
				<div id="scrollUpButton" className="hidden" onClick={() => { window.scrollTo({ top: 0, behavior: 'smooth' }); }} ref={this.scrollUpButtonRef}>
				...
				<div id="aboutChevron_down" className="chevron" onClick={this.handleSectionScroll}>
				...
				<div id="MTDandMTinfo">
					<div id="MTDinfo" className="aboutinfo">
						<div className="content">
							<h1>{general[this.state.lang].name} {general.year}</h1>
							<p className="ingress">{content[this.state.lang].MTDinfo.ingress1} {general[this.state.lang].name} {content[this.state.lang].MTDinfo.ingress2} {general.nameShort} {general.year} {content[this.state.lang].MTDinfo.ingress3}</p>
							<p>{content[this.state.lang].MTDinfo.body1}<br/><br/>{general.nameShort} {content[this.state.lang].MTDinfo.body2}</p>
						</div>
						<div className="image">
					...
					<div id="MTinfo" className="aboutinfo before">
						...
						<div className="content">
							...
							<p className="ingress">{content[this.state.lang].MTinfo.ingress}</p>
							<p>
								{content[this.state.lang].MTinfo.body1}<br/><br/>
								{content[this.state.lang].MTinfo.body2}<br/><br/>
								{content[this.state.lang].MTinfo.body3} <a href={general.http + content[this.state.lang].MTinfo.link1} target="_blank" rel="noopener noreferrer">{content[this.state.lang].MTinfo.link1}</a>
								{" " + content[this.state.lang].MTinfo.and} <a href={general.http + content[this.state.lang].MTinfo.link2} target="_blank" rel="noopener noreferrer">{content[this.state.lang].MTinfo.link2}</a>
							</p>
						</div>
					</div>
				</div>
				...
				<div id="FAIRinfo">
					...
					<div id="competitions" className="FAIRcontent sideBySide before">
						...
						<p>
							...
							<a href={content[this.state.lang].fairInfo["3"].link1.url} target="_blank" rel="noopener noreferrer">{content[this.state.lang].fairInfo["3"].link1.text}</a>
							{content[this.state.lang].fairInfo["3"].body2}
							<a href={content[this.state.lang].fairInfo["3"].link2.url} target="_blank" rel="noopener noreferrer">{content[this.state.lang].fairInfo["3"].link2.text}</a>
					...
					<div id="infoBeam" className="FAIRcontent beam before">
						<div className="aloneContent">
							...
							<p>
								{content[this.state.lang].fairInfo["4"].body}
							</p>
						</div>
						...
						<div className="aloneContent">
							...
							<p>
								{content[this.state.lang].fairInfo["5"].body}
							</p>
						</div>
					</div>
					...
					<div id="previousYears">
						...
						<div id="lecturer_1" className="lecturer">
							<h1 id="lecturerTitleSmall" className="before">{previous[this.state.lang].title}</h1>
							<div className="lecturerImage" style={{ backgroundImage: `url(${lecturePics[0]})` }}>
							...
							<div className="lecturerInfo left before">
								...
								<a href={previous[this.state.lang]["1"].link} target="_blank" rel="noopener noreferrer">{previous[this.state.lang]["1"].link}</a>
							</div>

							<div id="lecturer_2" className="lecturer before">
								<div className="lecturerImage" style={{ backgroundImage: `url(${lecturePics[1]})` }}>
								...
								<div className="lecturerInfo right before">
									...
									<a href={previous[this.state.lang]["2"].link} target="_blank" rel="noopener noreferrer">{previous[this.state.lang]["2"].link}</a>
								</div>

								<div id="lecturer_3" className="lecturer before">
									<div className="lecturerImage" style={{ backgroundImage: `url(${lecturePics[2]})` }}>
										<h2>{previous[this.state.lang]["3"].title}</h2>
									</div>
									<div className="lecturerInfo left before">
										<h1>{previous[this.state.lang]["3"].title}</h1>
										<p>{previous[this.state.lang]["3"].body}</p>
										<a href={previous[this.state.lang]["3"].link} target="_blank" rel="noopener noreferrer">{previous[this.state.lang]["3"].link}</a>
									</div>

									<div id="lecturer_4" className="lecturer before">
										<div className="lecturerImage" style={{ backgroundImage: `url(${lecturePics[3]})` }}>
											<h2>{previous[this.state.lang]["4"].title}</h2>
										</div>
										<div className="lecturerInfo right before">
											<h1>{previous[this.state.lang]["4"].title}</h1>
											<p>{previous[this.state.lang]["4"].body}</p>
											<a href={previous[this.state.lang]["4"].link} target="_blank" rel="noopener noreferrer">{previous[this.state.lang]["4"].link}</a>
										</div>
						...
						<div id="videoWrap">
							<div id="videoContainer">
								{}<FacebookPlayer
									url='https://www.facebook.com/medieteknikdagarna/videos/3180624732076649/'
									config={{
										facebook: {
