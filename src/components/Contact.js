...
const groupMembers = [
	"manager", "economy", "business", "fair", "assistant", "lecture", "banquet", "tech", "pr", "coordinator", "web", "app", "print"
];
...
class GeneralContent extends Component {
	...
	render() {
		let questions = content[this.state.lang]["0"].questions.map((item, index) => {
			let mail = `mailto: ${group[item.pos].email}`;
			return (
				<div className="question" key={index}>
					<h2>{item.title}</h2>
					<p>{item.body1 + group[item.pos].name.split(" ")[0] + item.body2}</p>
					<a className="mailTo" href={mail}><FontAwesomeIcon icon={['fas', 'paper-plane']} className="fa"/>{group[item.pos].email}</a>
				</div>
			);
		});
    ...
    return (
			<div id="generalContentWrap" style={{ width: `${this.state.containerWidth}%`, height: this.state.containerHeight }}>
				<div id="generalContentContainer" style={{ width: this.state.wrapWidth }}>
					...
					<div id="maps">
						<div className="mapContainer">
							...
							<div className="address">
								<h2>{content[this.state.lang]["0"].addresses.visiting.title}</h2>
						...
						<div className="mapContainer">
							...
							/>
							<div className="address">
								<h2>{content[this.state.lang]["0"].addresses.delivery.title}</h2>
...
class GroupContent extends Component {
				...
				<div className="personContainer" key={index}>
					<div className="person">
						<div className="imageContainer">
							<div className="image" style={{ backgroundImage: `url(${group[item].pic})` }} alt={group[item].pos[this.state.lang]}/>
						</div>
						<div className="info">
							<h2>{group[item].name}</h2>
							<h3>{group[item].pos[this.state.lang]}</h3>
							<a href={email} target="_top"><FontAwesomeIcon icon={['fas', 'paper-plane']} className="fa"/>{group[item].email}</a><br/>
							<a href={tel} target="_top"><FontAwesomeIcon icon={['fas', 'phone-alt']} className="fa"/>{group[item].tel}</a>
						</div>
					</div>
				</div>
			);
    ...
    return (
			<div id="groupContentWrap" style={{ width: `${this.state.containerWidth}%`, height: this.state.containerHeight }}>
				<div id="groupContentContainer" style={{ width: this.state.wrapWidth }}>
					<Link to={settings.url + "contact/"} className="goBack">
						<FontAwesomeIcon icon={['fas', 'chevron-circle-left']} className="fa"/>
					</Link>
					<div className="contentTitle">
...
class PressContent extends Component {
	...
	render() {
		let body = content[this.state.lang]["2"].pressinfo.body.split("\n").map((item, index) => {
			return (
				<p key={index}>{item}</p>
			);
		});
    ...
    return (
			<div id="pressContentWrap" style={{ width: `${this.state.containerWidth}%`, height: this.state.containerHeight }}>
				<div id="pressContentContainer" style={{ width: this.state.wrapWidth }}>
					...
					<div id="pressinfo">
						...
						<div id="right">
							...
							<p>{group.pr.name}, {group.pr.pos[this.state.lang]}</p>
							<a href={prMail}><FontAwesomeIcon icon={['fas', 'paper-plane']} className="fa"/>{group.pr.email}</a>
							<br/>
							<a href={prTel}><FontAwesomeIcon icon={['fas', 'phone-alt']} className="fa"/>{group.pr.tel}</a>
						</div>
					</div>
					<div id="below">
