...
var WARNING_DISPLAY_TIMEOUT = 1000;
...
var Map = function (_Component) {
  ...
  function Map(props) {
    ...
    _this.handleWheel = function (event) {
      ...
      if (!metaWheelZoom || event.ctrlKey) {
        event.preventDefault();
    ...
    _this.zoomAroundMouse = function (zoomDiff, event) {
      ...
      const zoomMin = (zoom === minZoom && zoomDiff < 0);
      const zoomMax = (zoom === maxZoom && zoomDiff > 0);

      if (!_this._mousePosition || zoomMin || zoomMax) {
        return;
      }
  ...
  Map.prototype.componentDidUpdate = function componentDidUpdate(prevProps) {
    ...
    const centerBool1= (this.props.center[0] === prevProps.center[0] && this.props.center[1] === prevProps.center[1]);
    const centerBool2 = (!this.props.center || centerBool1);

    if (centerBool2 && this.props.zoom === prevProps.zoom) {
  ...
  Map.prototype.renderWarning = function renderWarning() {
    ...
    const metaZoomBool = (metaWheelZoom && metaWheelZoomWarning);
    const twoFingerDragBool = (twoFingerDrag && twoFingerDragWarning);

    if (metaZoomBool || twoFingerDragBool) {
      var style = {
        position: 'absolute',
        top: 0,
        left: 0,
