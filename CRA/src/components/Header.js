...
class Header extends Component {
	...
	showDropdown() {
		if(!isMobile && Math.max(document.documentElement.clientWidth, window.innerWidth || 0) > 850) {
		...
		if (oldNav.toString() !== nav.toString()) {
			...
			switch (nav[3]) {
				...
				case "about":
					...
					this.setState({ borderBottom: {borderBottom: "4px solid transparent"} });
	...
	render() {
		...
		if(this.state.loggedin) {
			...
			accountIcon = (
				<Link to={settings.url + "login"} id="accountIcon">
					<FontAwesomeIcon icon={['fa', 'user-circle']} className="fa fa-user-circle"/>
					<FontAwesomeIcon icon={['fa', 'sort-down']} className="fa fa-sort-down"/>
		...
		if(isMobile || Math.max(document.documentElement.clientWidth, window.innerWidth || 0) < 850) {
			return (
				<header id="headerHamburger" className={this.state.transparentClass} ref={this.headerRef}>
				<div id="navis">
					<div id="banner">
						...
						<FontAwesomeIcon icon={['fas', 'bars']} className={this.state.scrolledClass} id="openNavButton" onClick={this.showHamburger}/>
					</div>
					...
					<div id="navPopOut" className={this.state.popOutNavClass}>
						...
						<nav id="nav">
							...
							<Link to={settings.url + "about/"} id="about" onClick={this.hideHamburger}>
								{content[this.state.lang].about.title}
							</Link>
							<Link to={settings.url + "contact/"} id="contact" onMouseEnter={this.showDropdown} onMouseLeave={this.hideDropdown} onClick={(e) => { this.hideDropdown(e); e.currentTarget.classList.remove("hover"); this.hideHamburger()}}>
