...
const staticSites = [
	...
	{url: "previous", comp: Previous, active: true},
...
class App extends Component {
	render() {
		const route = sites.map((item, i) => {
			if(item.active) {
				return <Route exact path={settings.url + item.url} component={item.comp} key={i}/>;
			}
			return null;
		});
...
class Static extends Component {
	...
	render() {
		...
		if(staticSites.find(item => item.url === this.state.nav[3] && item.active)) {
			header = (
				<Header
			...
			acceptPolicy = (
				<div id="acceptPolicy">
					<div id="acceptPolicyText">
						{general[this.state.lang].acceptPolicy} <Link to={settings.url + general[this.state.lang].acceptPolicyLink.url + "/"}>{general[this.state.lang].acceptPolicyLink.text}</Link>
					</div>
					<div id="acceptPolicyButton" onClick={this.acceptPolicy}>{general[this.state.lang].acceptPolicyButton}</div>
				</div>
			);
		}
			...
			<div id="static">
				...
				<main id="wrapper">
					<Switch>
						...
						<Route exact path={settings.url + ""} render={(props) => <Home {...props} lang={this.state.lang} />}/>
						<Route exact path={settings.url + "about/"} render={(props) => <About {...props} lang={this.state.lang} />}/>
						<Route exact path={settings.url + "about/privacy-policy/"} render={(props) => <PrivacyPolicy {...props} lang={this.state.lang} />}/>
						<Route exact path={settings.url + "about/:subpage/"} render={(props) => <About {...props} lang={this.state.lang} />}/>
						<Route exact path={settings.url + "contact/"} render={(props) => <Contact {...props} lang={this.state.lang} />}/>
