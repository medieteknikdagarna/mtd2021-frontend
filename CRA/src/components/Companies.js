...
class Companies extends Component {
	...
	openModal() {
		if(!this.state.modalOpen) {
			let modalBackgroundStyle = JSON.parse(JSON.stringify(this.state.modalBackgroundStyle));
	...
	async loadCompanies() {
		let goldCompanies = [], silverCompanies = [], bronzeCompanies = [], regularCompanies = [], startupCompanies = [];
		const filter = this.state.filter;

		for (let i = companyInformation.companies.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [companyInformation.companies[i], companyInformation.companies[j]] = [companyInformation.companies[j], companyInformation.companies[i]];
    }

		await companyInformation.companies.forEach((item, index) => {
			switch (item.level) {
				case "gold":
					if(!filter.exjobb && !filter.praktik && !filter.trainee && !filter.sommarjobb && !filter.anstallning) { goldCompanies.push(item); }
					else if(filter.exjobb === item.offers.exjobb) { goldCompanies.push(item); }
					else if(filter.praktik === item.offers.praktik) { goldCompanies.push(item); }
					else if(filter.trainee === item.offers.trainee) { goldCompanies.push(item); }
					else if(filter.sommarjobb === item.offers.sommarjobb) { goldCompanies.push(item); }
					else if(filter.anstallning === item.offers.anstallning) { goldCompanies.push(item); }
					break;
				case "silver":
					if(!filter.exjobb && !filter.praktik && !filter.trainee && !filter.sommarjobb && !filter.anstallning) { silverCompanies.push(item); }
					else if(filter.exjobb === item.offers.exjobb) { silverCompanies.push(item); }
					else if(filter.praktik === item.offers.praktik) { silverCompanies.push(item); }
					else if(filter.trainee === item.offers.trainee) { silverCompanies.push(item); }
					else if(filter.sommarjobb === item.offers.sommarjobb) { silverCompanies.push(item); }
					else if(filter.anstallning === item.offers.anstallning) { silverCompanies.push(item); }
					break;
				case "bronze":
					if(!filter.exjobb && !filter.praktik && !filter.trainee && !filter.sommarjobb && !filter.anstallning) { bronzeCompanies.push(item); }
					else if(filter.exjobb === item.offers.exjobb) { bronzeCompanies.push(item); }
					else if(filter.praktik === item.offers.praktik) { bronzeCompanies.push(item); }
					else if(filter.trainee === item.offers.trainee) { bronzeCompanies.push(item); }
					else if(filter.sommarjobb === item.offers.sommarjobb) { bronzeCompanies.push(item); }
					else if(filter.anstallning === item.offers.anstallning) { bronzeCompanies.push(item); }
					break;
				case "regular":
					if(!filter.exjobb && !filter.praktik && !filter.trainee && !filter.sommarjobb && !filter.anstallning) { regularCompanies.push(item); }
					else if(filter.exjobb === item.offers.exjobb) { regularCompanies.push(item); }
					else if(filter.praktik === item.offers.praktik) { regularCompanies.push(item); }
					else if(filter.trainee === item.offers.trainee) { regularCompanies.push(item); }
					else if(filter.sommarjobb === item.offers.sommarjobb) { regularCompanies.push(item); }
					else if(filter.anstallning === item.offers.anstallning) { regularCompanies.push(item); }
					break;
				case "startup":
					if(!filter.exjobb && !filter.praktik && !filter.trainee && !filter.sommarjobb && !filter.anstallning) { startupCompanies.push(item); }
					else if(filter.exjobb === item.offers.exjobb) { startupCompanies.push(item); }
					else if(filter.praktik === item.offers.praktik) { startupCompanies.push(item); }
					else if(filter.trainee === item.offers.trainee) { startupCompanies.push(item); }
					else if(filter.sommarjobb === item.offers.sommarjobb) { startupCompanies.push(item); }
					else if(filter.anstallning === item.offers.anstallning) { startupCompanies.push(item); }
					break;
				default:
					break;
			}
		...
		this.setState({ goldCompanies, silverCompanies, bronzeCompanies, regularCompanies, startupCompanies });
	}
	...
	render() {
		let goldCompanies = null, silverCompanies = null, bronzeCompanies = null, regularCompanies = null, startupCompanies = null;
		...
		if(this.state.company) {
			renderCompany = ( <Company lang={this.state.lang} companyInfo={this.state.company}/> );
		}

    return (
			<div id="companiesWrap">
				<div id="modalBackground" style={this.state.modalBackgroundStyle} onMouseDown={this.closeModal}>
					<div id="modal" style={this.state.modalStyle} onMouseDown={((e) => {e.stopPropagation();})}>
						<div id="modalCloseButton">
							<FontAwesomeIcon icon={['fas', 'times']} className="fa" onMouseDown={this.closeModal}/>
						</div>
						{renderCompany}
					</div>
				</div>
